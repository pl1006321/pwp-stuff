...
def apply_overlay(frame): 
    ...
    hsv = cv2.cvtColor(cropped, cv2.COLOG_BGR2HSV)
...
def update_vid_stream(element1, element2): 
    while True:
        ...
        # stream = cv2.resize(frame, (400, 240))
        # overlay = cv2.resize(overlay, (400, 240))
...
def create_robot_gui(user):  # sets up tkinter window for controlling robot if login is successful
    ...
    buttons_panel.grid(row=5, column=2, rowspan=1, padx=80, pady=60, sticky='NWSE')
    ...
    log_panel.grid(row=6, column=2, rowspan=1, padx=10, pady=5, sticky='NS')
    ...
    open_file_button = Button(log_panel, text='open log file', command=open_log_file)
    open_file_button.grid(row=2, padx=2.5, pady=5, ipadx=5, ipady=5,)

    stream_elem = Label(vid_stream_panel, text='vid stream')
    stream_elem.grid(padx=20, pady=10)

    overlay_elem = Label(vid_overlay_panel, text='vid overlay')
    overlay_elem.grid(padx=70, pady=20)

    video_thread = Thread(target=update_vid_stream, args=(stream_elem,overlay_elem))
    video_thread.daemon = True 
    video_thread.start()

    forward = Button(buttons_panel, text='move forward',)
    ...
    left = Button(buttons_panel, text='move left',)
    ...
    # play = Button(buttons_panel, text='play', command=lambda: post_direction('play', user, text_area))
    # play.grid(row=2, column=2, padx=2.5, pady=5, ipadx=5, ipady=5, sticky='we')

    stop = Button(buttons_panel, text='stop', command=lambda: post_direction('stop', user, text_area))
    stop.grid(row=2, column=2, padx=2.5, pady=5, ipadx=5, ipady=5, sticky='we')

    right = Button(buttons_panel, text='move right', )
    ...
    backward = Button(buttons_panel, text='move backward',)
